...
function emotionalMessage(payload, levelValue) {
  ...
  if (levelValue === "Low" || levelValue === "Mild")
    return `${name}'s cardiovascular profile is reassuring â€” the current pattern is protective and worth maintaining.`;
  ...
  return `${name}'s profile shows elevated risk that deserves calm, structured attention â€” not panic, but purposeful action.`;
}
...
function attackScenario(payload, riskScore, refinementPoints = 0) {
  ...
  return {
    ...
    summary: label === "lower"
      ? "This looks more like a prevention opportunity than an immediate danger â€” but prevention is most valuable when acted on early."
...
function explanations(payload) {
  const items = [];
  if (payload.smoking === "yes") items.push(["Smoking", "Smoking is the strongest modifiable cardiovascular risk factor â€” it damages endothelial cells and promotes thrombosis."]);
...
function dailyStory(payload, patterns) {
  ...
  if (!story.length) {
    if (payload.activity_level !== "low" && payload.stress !== "high") {
      story.push(`${name}'s daily rhythm shows more stability than strain â€” the current pattern is protective.`);
    } else {
...
function bodySignals(payload) {
  const signals = [];
  if (payload.sitting === "high") signals.push("Stiffness or circulatory sluggishness after prolonged sitting â€” response to sustained venous pooling.");
  if (payload.stress === "high") signals.push("Persistent tension or fatigue that doesn't resolve with rest â€” chronic sympathetic activation.");
  if (payload.sleep_quality === "poor") signals.push("Waking unrefreshed, daytime fog, reduced stamina â€” markers of incomplete overnight recovery.");
  if (payload.exertion_response === "strained") signals.push("Breathlessness or unusual fatigue during routine activity â€” reduced cardiac reserve.");
  ...
  if (!signals.length) signals.push("No concerning body signals detected â€” a positive finding worth protecting.");
  return signals.slice(0, 5);
}

function foodGuidance(payload) {
  const leanInto = [
    "Whole foods: vegetables, legumes, whole grains, nuts, fish â€” the Mediterranean pattern shows consistent CVD benefit.",
    "Potassium-rich foods (bananas, sweet potatoes, spinach) to counterbalance sodium's effect on BP.",
    "Consistent meal timing â€” irregular eating disrupts glucose regulation and lipid metabolism.",
  ];
  const easeOff = [
    "Processed foods high in sodium (>600mg/serving) â€” the primary dietary driver of hypertension.",
    "Trans fats and deep-fried foods â€” directly raise LDL and lower HDL.",
    "Added sugars and refined carbs â€” drive triglyceride elevation and insulin resistance.",
  ];
  if (payload.systolic_bp != null && payload.systolic_bp >= 130) leanInto.unshift(`With BP at ${payload.systolic_bp} mmHg: DASH diet pattern â€” proven to reduce systolic BP 8â€“14 mmHg.`);
  if (payload.cholesterol != null && payload.cholesterol >= 240) easeOff.unshift(`With cholesterol at ${payload.cholesterol} mg/dL: reduce saturated fat â€” each 1% reduction lowers LDL ~2 mg/dL.`);
...
function symptomWatch(payload) {
  const watch = [];
  if (payload.exertion_response !== "easy") watch.push("Track whether effort tolerance worsens â€” this is a key cardiovascular warning signal.");
  if (payload.stress === "high") watch.push("Monitor for chest tightness during stress â€” stress cardiomyopathy can mimic cardiac events.");
  ...
  if (payload.sleep_quality === "poor") watch.push("Pay attention to morning blood pressure â€” poor sleep often causes non-dipping patterns.");
  if (!watch.length) watch.push("Continue noticing how your body responds to effort â€” catching changes early is prevention.");
  return watch.slice(0, 4);
}

function habitFocus(payload) {
  const focus = [];
  if (payload.activity_level === "low") focus.push("Movement is the single most protective lifestyle factor â€” 150 min/week reduces CVD risk by 30% (WHO).");
  if (payload.diet === "frequent") focus.push("Dietary pattern is driving metabolic load â€” simplification is the realistic goal.");
  if (payload.stress === "high") focus.push("The nervous system needs daily decompression â€” without it, cortisol stays elevated.");
  if (payload.sleep_quality === "poor") focus.push("Sleep is cardiovascular recovery time â€” protecting it has disproportionate returns.");
  if (payload.sitting === "high") focus.push("Sedentary time is an independent risk factor â€” even exercisers who sit 8+ hr/day have elevated risk.");
  return focus.slice(0, 4).length ? focus.slice(0, 4) : ["Your current habits are protective â€” focus on sustaining them long-term."];
}
...
function doctorVisitScript(payload, levelValue, issues) {
  ...
  if (payload.systolic_bp != null && payload.systolic_bp >= 130) items.push(`Blood pressure recorded: ${payload.systolic_bp} mmHg â€” please verify.`);
  if (payload.fasting_glucose != null && payload.fasting_glucose >= 100) items.push(`Fasting glucose: ${payload.fasting_glucose} mg/dL â€” consider confirmatory testing.`);
  if (payload.exertion_response !== "easy") items.push("Exertional symptoms reported during routine activity â€” cardiac evaluation may be valuable.");
...
function suggestions(payload, levelValue, issues) {
  ...
  const doctorGuidance = {
    Low: "Stay consistent and schedule preventive checks every 1â€“2 years.",
...
function careFlags(payload, levelValue) {
  const flags = [];
  if (levelValue === "High" || levelValue === "Very High") flags.push("This risk level warrants medical consultation â€” early intervention has the highest return.");
  ...
  if (payload.diabetes === "yes") flags.push("Diabetes fundamentally changes CVD risk management â€” structured prevention is essential.");
  ...
  if (payload.smoking === "yes") flags.push("Smoking cessation at any age reduces cardiovascular risk â€” it's the single most impactful change.");
  if (payload.fasting_glucose != null && payload.fasting_glucose >= 126) flags.push(`Fasting glucose of ${payload.fasting_glucose} mg/dL is in diabetes range (â‰¥126, ADA) â€” needs medical confirmation.`);
  if (!flags.length) flags.push("No urgent flags â€” maintain current patterns and schedule regular preventive check-ups.");
...
function refinementDeltaV2(answers) {
  const rules = {
    "chest_tightness:yes": { delta: 7, text: "Chest pressure confirmed - this elevates the urgency of cardiac evaluation." },
    "breathless_easy:yes": { delta: 6, text: "Exertional breathlessness confirmed - suggests reduced cardiac or pulmonary reserve." },
    "palpitations:yes": { delta: 4, text: "Palpitations confirmed - rhythm assessment may be warranted." },
    "ankle_swelling:yes": { delta: 5, text: "Peripheral edema confirmed - circulatory assessment should be prioritized." },
    "dizzy_standing:yes": { delta: 4, text: "Orthostatic symptoms confirmed - blood pressure regulation needs evaluation." },
    "chest_pain_frequency:stress_only": { delta: 3, text: "Chest pressure appears during stress - worth separating stress load from cardiac triggers." },
    "chest_pain_frequency:stairs": { delta: 6, text: "Chest pressure with stairs or brisk walking suggests a more important exertional symptom pattern." },
    "chest_pain_frequency:rest": { delta: 9, text: "Chest pressure even at rest raises the need for prompt medical review." },
    "chest_pain_duration:few_minutes": { delta: 2, text: "Chest discomfort lasting a few minutes adds weight to the symptom pattern." },
    "chest_pain_duration:ten_plus": { delta: 4, text: "Symptoms lasting 10 minutes or more increase the urgency of clinical follow-up." },
    "breathlessness_pattern:stairs": { delta: 3, text: "Breathlessness on stairs suggests reduced exertional reserve." },
    "breathlessness_pattern:flat_walk": { delta: 6, text: "Breathlessness during a normal flat walk suggests more meaningful functional limitation." },
    "breathlessness_pattern:rest": { delta: 8, text: "Breathlessness even at rest is a higher-risk symptom pattern." },
    "recovery_after_effort:two_to_ten": { delta: 2, text: "Slower recovery after exertion points to reduced conditioning or recovery reserve." },
    "recovery_after_effort:over_ten": { delta: 4, text: "Recovery taking more than 10 minutes suggests a stronger exertional burden." },
    "palpitations_pattern:occasional": { delta: 1, text: "Occasional rhythm awareness was reported." },
    "palpitations_pattern:weekly": { delta: 3, text: "Weekly palpitations suggest a recurring rhythm complaint worth tracking." },
    "palpitations_pattern:with_dizziness": { delta: 5, text: "Palpitations with dizziness increase the need for rhythm-focused evaluation." },
    "leg_swelling_pattern:sometimes": { delta: 2, text: "Intermittent evening swelling suggests some fluid retention or venous pooling." },
    "leg_swelling_pattern:most_days": { delta: 5, text: "Frequent swelling suggests persistent fluid retention and raises follow-up priority." },
    "snoring_pattern:sometimes": { delta: 1, text: "Snoring symptoms may be reducing sleep quality and overnight recovery." },
    "snoring_pattern:often": { delta: 3, text: "Frequent snoring or gasping raises the possibility of sleep-disordered breathing." },
    "salt_intake:moderate": { delta: 1, text: "Diet appears to include a moderate sodium load." },
    "salt_intake:high": { delta: 3, text: "High sodium intake is likely adding pressure to blood pressure control." },
    "fruit_veg_servings:0_1": { delta: 3, text: "Low fruit and vegetable intake reduces dietary cardiovascular protection." },
    "fruit_veg_servings:2_3": { delta: 1, text: "Fruit and vegetable intake is only partly protective right now." },
    "sugary_drinks:few_weekly": { delta: 1, text: "Sugary drinks show up often enough to add metabolic load." },
    "sugary_drinks:daily": { delta: 3, text: "Daily sugary drinks likely add a meaningful glucose and triglyceride burden." },
    "cigarettes_per_day:1_5": { delta: 2, text: "Daily smoking exposure is present even at the lower range." },
    "cigarettes_per_day:6_10": { delta: 4, text: "Daily smoking volume is high enough to steepen cardiovascular risk." },
    "cigarettes_per_day:10_plus": { delta: 6, text: "Heavy daily smoking is a major accelerating cardiovascular factor." },
  };
  ...
  Object.entries(answers || {}).forEach(([key, value]) => {
    if (key === "sleep_hours" && typeof value === "number") {
      if (value <= 5) {
        refinementPoints += 3;
        confirmed.push("Sleep duration appears strongly insufficient, which can worsen blood pressure, stress response, and recovery.");
      } else if (value <= 6) {
        refinementPoints += 1;
        confirmed.push("Sleep duration appears slightly low, which may be limiting overnight recovery.");
      }
      return;
    }
    ...
    if (!rule) return;
    refinementPoints += rule.delta;
    confirmed.push(rule.text);
  });

  return { refinementPoints, confirmed };
}

export function refineOffline(payload, currentResult, answers) {
  const base = currentResult || predictOffline(payload, false);
  const { refinementPoints, confirmed } = refinementDeltaV2(answers);
  ...
  const refined = {
    ...
    refinement_summary: confirmed.length
      ? "The adaptive follow-up interview added symptom frequency, recovery, sleep, and habit context that sharpens the clinical picture."
      : "No major additional warning pattern was confirmed - that still helps narrow the picture.",
  };
